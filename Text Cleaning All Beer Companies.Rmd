...
#Extract the images/emojis.

Information from https://lyons7.github.io/portfolio/2017-10-04-emoji-dictionary/

We had to use the package "DataCombine", because stringr was not working with the Unicode for emojis. Once we convert all of the emojis to regular text, we will switch back to stringr for the analysis.


```{r}
#Emojis in capital letters with no spaces
emoticons <- read.csv("Emoji Dictionary 2.1.csv")
sample_n(emoticons, 1)
names(emoticons)
...
tweets$text2 <- iconv(tweets$text, from = "latin1", to = "ascii", 
                    sub = "byte")
head(tweets$text2)

tweets <- FindReplace(data = tweets, Var = "text2", 
                      replaceData = emoticons,
                      from = "R_Encoding", to = "Name", 
                      exact = FALSE)

emojipattern <- as.character(emoticons$R_Encoding)
emojis <- str_extract_all(tweets$text, pattern = emojipattern)
head(emojis)


##Emojis with spaces
emoticonspace <- read.csv("emojis workbook.csv")
sample_n(emoticonspace, 1)
names(emoticonspace)
